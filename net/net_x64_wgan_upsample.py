...
# kernel size / stride = int
# out = (in -1) * stride - 2 * padding + kernel_size + out_padding

class Generator(nn.Module):
    def __init__(self,):
        super(Generator, self).__init__()
        self.block_in = nn.Sequential(  # [batch, 136, 1, 1]
            nn.Conv2d(in_channels=136, out_channels=128, kernel_size=(1, 1), padding=0),
        ...
        self.block_out = nn.Sequential(
            ...
            nn.Conv2d(in_channels=128, out_channels=3, kernel_size=(3, 3), padding=1),
            nn.Sigmoid()  # [batch, 3, 64, 64]
        )
