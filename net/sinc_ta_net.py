...
class AudioNet(nn.Module):
    def __init__(self, options):
        ...
        self.fc_temporal_1 = nn.Conv1d(in_channels=60, out_channels=60, kernel_size=1)
        self.fc_temporal_2 = nn.Conv1d(in_channels=60, out_channels=60, kernel_size=1)
    ...
    def forward(self, x):
        x = self.CNN_net(x)
        x_att = self.fc_temporal_1(x)
        x_att = self.relu(x_att)
        x_att = self.softmax(x_att)
        x = x * x_att
        x_att = self.fc_temporal_2(x)
        x_att = self.relu(x_att)
        x_att = self.softmax(x_att)
        x = x * x_att
...
================================================================
         LayerNorm-1                 [-1, 1600]           3,200
     SincConv_fast-2             [-1, 80, 1350]             160
         LayerNorm-3              [-1, 80, 450]          72,000
         LeakyReLU-4              [-1, 80, 450]               0
           Dropout-5              [-1, 80, 450]               0
            Conv1d-6              [-1, 60, 446]          24,060
         LayerNorm-7              [-1, 60, 148]          17,760
         LeakyReLU-8              [-1, 60, 148]               0
           Dropout-9              [-1, 60, 148]               0
           Conv1d-10              [-1, 60, 144]          18,060
        LayerNorm-11               [-1, 60, 48]           5,760
        LeakyReLU-12               [-1, 60, 48]               0
          Dropout-13               [-1, 60, 48]               0
           Conv1d-14               [-1, 60, 48]           3,660
          SoftMax-15               [-1, 60, 48]               0
           Conv1d-16               [-1, 60, 48]           3,660
          SoftMax-17               [-1, 60, 48]               0
================================================================
Total params: 148,320
Trainable params: 148,320
Non-trainable params: 0
----------------------------------------------------------------
Input size (MB): 0.006104
Forward/backward pass size (MB): 2.287292
Params size (MB): 0.565796
Estimated Total Size (MB): 2.859192
----------------------------------------------------------------
output shape:  torch.Size([1, 2880])
"""